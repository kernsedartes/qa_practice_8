...
def moderator_token(client: ApiClient, admin_token: str):
    ...
    ), f"Не удалось зарегистрировать модератора: {resp.text}"
    ...
    role_resp = client.update_account_role(
        account_id, "moderator", token=admin_token
    )
    assert (
        role_resp.status_code == 200
    ), f"Не удалось назначить роль модератора: {role_resp.text}"
...
def registered_user(client: ApiClient, admin_token: str):
    ...
    assert (
        resp.status_code == 200
    ), f"Не удалось зарегистрировать тестового юзера: {resp.text}"
