...
| Эндпоинт | Ожидалось | Фактически | Описание |
|---|---|---|---|
| `POST /api/auth/change-password` | 401 | 400 | При неверном старом пароле возвращает 400 вместо 401 |
| `DELETE /api/profiles/{id}` | 404 | 500 | При удалении несуществующего аккаунта возвращает 500 |
